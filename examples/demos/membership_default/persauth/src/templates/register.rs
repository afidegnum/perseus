...
#[cfg(client)]
async fn register_user(email: &str, password: &str) -> Result<ApiResponse<SessionData>, String> {
...
async fn confirm_otp(
    ...
    let response = Request::post("/api/auth/confirm")
        .json(&serde_json::json!({
            "code": code,
...
async fn resend_otp(
    session_id: i32,
    session_verifier: &str,
) -> Result<ApiResponse<SessionData>, String> {
    use gloo_net::http::Request;

    let response = Request::post("/api/auth/resend-otp")
        .json(&serde_json::json!({
            "session_id": session_id,
            "session_verifier": session_verifier
        }))
